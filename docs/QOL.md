...
- **The `@` reference appeared in the CLI's input box, but Enter doesn't send it:** keyboard focus is still in the panel. Ctrl+V is bound across the whole panel, so pasting anywhere in it (or clicking **Paste**) stages the attachment and pushes the `@` reference - but the panel keeps focus, and a floating panel can sit on top of the terminal, so the click you thought landed in the terminal landed in the panel. Click the terminal's input line, then type and send. Corollary: an `@` reference in the input box always came from the panel - pasting an image straight into the terminal does nothing at all (see below).
