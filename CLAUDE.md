...
- `src/ClaudeCodeVS/Attachments/` - `AttachmentService` (SelectionService-pattern static, `Attach(server)` from BridgeHost): the panel attach tray's engine. Stages pasted screenshots/dropped files (in-workspace = referenced in place; else copied to `<ws>\.claude\attachments\` behind a self-ignoring `*` gitignore, 7-day prune) and pushes each as an **`at_mentioned`** notification so the reference lands in the CLI composer. ONE framework for all formats: image/pdf/text read directly, BMP transcoded to PNG, everything else (xlsx/mp4/zip) mentioned with a 🧰 needs-tool label - never hard-rejected. Per-item token estimates ((w×h)/750 images, bytes/4 text). UI = the 📎 card in `ClaudeToolWindowControl` (drop target, Paste button, chips: click = re-mention, ✕ = remove-and-delete-copy). The tray is **workspace-scoped**: `WorkspaceWatcher` calls `Clear()` on every workspace change (solution open/close, folder open/close) - chips carry workspace-relative mention paths and live in that workspace's staging folder, so they can't survive into the next one.
