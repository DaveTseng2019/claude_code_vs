...
- `src/ClaudeCodeVS/Ui/` - dockable panel (BridgeStatus state, ClaudeToolWindowControl WPF, ReasonDialog) + `Notifier` (turn-finished / needs-input notifications: ONE main-window InfoBar, superseded by the next, + a bounded taskbar flash when VS is backgrounded; turn-end rides the existing Stop hook's `/usage` POST via `IdeWebSocketServer.StopReceived` - no extra hook; gated by the panel's Notify toggle, default ON).
